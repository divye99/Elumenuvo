...
// Supabase's SQL Editor rejects queries around the 1MB mark, so the output is
// split into self-contained part files (~900 products each, ~450KB). Each part
// upserts its own mappings AND snapshots, so parts can run in any order but
// only after 0041/0042 exist.
const PER_PART = 900;
const partCount = Math.ceil(maps.length / PER_PART);
const header = (n) => `-- ═══════════════════════════════════════════════════════════════
-- 0043 · Havells own-store mappings + first price snapshots · part ${n} of ${partCount}
...
--
-- RUN AFTER ALL 0041 PARTS AND 0042 (foreign key on product_id). Parts are
-- self-contained and idempotent (upsert on product_id+source), any order.
-- ═══════════════════════════════════════════════════════════════\n\n`;

for (let i = 0; i < partCount; i++) {
  const mapChunk = maps.slice(i * PER_PART, (i + 1) * PER_PART);
  const priceChunk = prices.slice(i * PER_PART, (i + 1) * PER_PART);
  const sql = header(i + 1) + `insert into public.competitor_map (product_id, source, competitor_code, competitor_url, unit_factor, note, match_method, approval, item_condition) values
${mapChunk.join(",\n")}
...
`;
  writeFileSync(path.join(MIG, `0043_havells-ownstore-map-part${i + 1}.sql`), sql);
}
console.log(`0043 written in ${partCount} parts: ${maps.length} mappings + ${prices.length} snapshots (${missing} ids had no live listing, correctly skipped).`);
