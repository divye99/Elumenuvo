            ...
            <div key={it.id} style={{ display: "flex", gap: 14, alignItems: "center", padding: "14px 16px", borderTop: i ? "1px solid #F0F2F6" : undefined }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12, color: "#8A93A6" }}>
                  {it.brand} · {fmt(baseExGst(unitPriceFor(it.price, it.qty, it.cat), it.cat, it.gstRate))}+GST/{it.unit}
                  {it.qty >= WHOLESALE_MIN_QTY && wholesaleEligible(it.cat) && <span style={{ marginLeft: 6, fontSize: 10.5, fontWeight: 700, color: "#1F9D63", background: "#E6F5EE", padding: "2px 7px", borderRadius: 6 }}>wholesale −5% applied</span>}
                </div>
              </div>
              ...
              <div style={{ width: 90, textAlign: "right", fontFamily: GROTESK, fontWeight: 700, fontSize: 14 }}>{fmt(baseExGst(unitPriceFor(it.price, it.qty, it.cat), it.cat, it.gstRate) * it.qty)}</div>
