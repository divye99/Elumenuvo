  ...
  return (
    <div>
      ...
      <Section n={2} title="Upload your edited spreadsheet">
        ...
        {pending && !done && <div style={{ fontSize: 13, color: "#8A93A6", marginTop: 10 }}>Reading…</div>}
      </Section>

      {done && (
        <div style={{ background: "#E6F5EE", color: "#137a4b", borderRadius: 12, padding: "14px 16px", fontSize: 14, fontWeight: 600, marginBottom: 16 }}>
          ✓ {done}
        </div>
      )}
          ...
          {diffs.length === 0 ? (
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 16 }}>
                <button onClick={confirm} disabled={pending} style={{ ...btnPrimary, background: "#4E5BDC", cursor: pending ? "default" : "pointer", opacity: pending ? 0.7 : 1 }}>
                  {pending ? "Applying…" : `Apply ${diffs.length} change${diffs.length === 1 ? "" : "s"}`}
                </button>
                <button onClick={() => { setDiffs(null); setErrors([]); setFilename(""); }} disabled={pending} style={{ background: "none", border: "none", color: "#56627A", fontSize: 13.5, cursor: "pointer" }}>Cancel</button>
              </div>
            </>
          )}
        </Section>
