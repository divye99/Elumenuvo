      ...
      <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 6, WebkitOverflowScrolling: "touch" }}>
        {items.map((p) => (
          <div key={p.id} style={{ flex: "0 0 172px", background: "#fff", border: "1px solid #E8EBF1", borderRadius: 13, padding: 10, display: "flex", flexDirection: "column" }}>
            <Link prefetch={false} href={`/catalogue/${p.id}`} style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 86, marginBottom: 8 }}>
              {p.image ? (
            ...
            <Link prefetch={false} href={`/catalogue/${p.id}`} style={{ fontSize: 12, fontWeight: 600, color: "#19202E", lineHeight: 1.35, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", minHeight: 32 }}>
              {p.name}
            </Link>
            ...
            <button
              ...
              style={{ marginTop: "auto", background: added === p.id ? "#1F9D63" : "#1D2F8A", color: "#fff", border: "none", borderRadius: 8, padding: "8px 0", fontSize: 12, fontWeight: 700, cursor: "pointer", transition: "all .15s" }}
            >
