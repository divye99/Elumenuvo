...
KAM note: quote from the live price list; the automatic 15-unit wholesale rate is the floor, project volumes can go sharper line by line.`,
  },
  {
    ...
    summary: "One shared catalogue cache, keyed by an automatic change watermark; which surfaces follow it within a minute and which follow their ISR window.",
    tags: ["ops"],
    body: `## The shape
...
## Two layers, two freshness rules
1. The DATA layer (the chunks above, the Norisys and compare-key caches). A single row in public.catalogue_version (migration 0133) is bumped by statement-level triggers on every insert, update or delete of products or reviews, however the change was made: admin console, import, backfill script, raw SQL. The app reads that version at most once a minute per instance (a 3-second bounded probe; a known value is served at once and refreshed in the background), quantised to 5-minute buckets so a row-by-row script does not rebuild the cache per row, and keys every chunk entry by it. Net: any data change reaches the data layer within about a minute (at most ~6 during a long script), automatically, and new entries are written only when the catalogue actually changed. The six-hour window is only a safety net if the watermark cannot be read.
2. The PAGE layer (rendered HTML). List pages (home, catalogue, brand, category, collections, price lists) are ISR with a one-hour window: they pick up the new data layer on their next regeneration, so worst case an hour. Product pages are ISR for a day and read their own row directly, so the watermark does not reach them: an admin console save purges that product's page by path (src/lib/admin/actions.ts), and scripts pass the changed ids to POST /api/admin/revalidate (scripts/revalidate-catalogue.mjs id1 id2 ...), which purges each page by path and drops the products tag.
Admin console writes additionally call revalidateTag("products") and reset the instance memo, so the console sees its own change on the very next request.

## Never cache a failure
A PostgREST result that never reached the database (aborted at the 60-second ceiling, gateway 5xx) is surfaced as an ERROR, never as "no rows": a product page that treated it as "no such product" would cache a 404 for a day, and a list page would cache an empty catalogue. The render fails instead, ISR keeps the last good page, and the column fallback ladders stop at the first transport failure rather than stacking the ceiling per rung.

## Why not a five-minute window (21 Aug 2026)
...
## Worked example
An admin changes a Havells fan price at 11:02. The trigger bumps the version; the save drops the tag and purges that product page, so the console shows the new price at once, every other server's data layer within a minute, and the list pages on their next hourly regeneration. A backfill script rewrites 700 spec sheets at 02:00 and ends with "node scripts/revalidate-catalogue.mjs <the 700 ids>": the data layer is fresh by 02:01 and each of those product pages re-renders on its next visit.
...
## Alerts
- Down: an email to the owner (ADMIN_EMAIL, BCC the house inbox) with what failed and what to look at (Supabase restart vs Vercel deployment), once per hourly run while it lasts. Slow: one email per episode.
- Recovered: one email with the outage duration, only if an alert actually went out. The mail goes out BEFORE the result is stored, so it never waits behind a broken database.
- While the database itself is down nothing can be stored: the run keeps rows and state in memory and flushes them on the first healthy run; if the instance was cold meanwhile, one "down" row is reconstructed from the gap so uptime and incident counts still include the outage, and the recovery mail says so honestly. The admin page shows "No recent check" in red when nothing has been stored for two and a half hours, instead of a stale Healthy.
- If Vercel Bot Protection is ever set to Challenge, add a Bypass rule for the ElumeHealthMonitor user agent first, or every page reads HTTP 403 to the monitor.
...
- Edge (the bouncer, src/proxy.ts, rules in src/lib/bots.ts bouncerVerdict): refuses DISGUISES before any render or query, never a real person on an old device. Refused: scraper toolkits that say so (Lightpanda, Puppeteer, Playwright, PhantomJS) and anonymous HeadlessChrome; requests claiming Google Chrome/Chromium 89+ that send no Sec-CH-UA client-hint header (every real Chrome, Edge, Opera, Brave, Samsung Internet sends it; a script wearing "Chrome/120" does not); and Firefox 116 to 124 when the request sends no Fetch Metadata at all (a script; a real Firefox in that band is served and logged as soft). Exempt by design: WebViews and in-app browsers, the OEM browsers that ship as default on Indian phones (HeyTap on Oppo/Realme/OnePlus, Vivo, UC 13 and 15, Mi, Huawei/Honor, JioPages, Quark), Amazon Silk, TV shells, Qt apps, Windows 7 on Chrome 109 or Firefox 115, KaiOS, old iPhones, and every self-identified crawler, AI answer engine, previewer or monitor (including Vercel's own deployment screenshot and Lighthouse). Known humans skip the bouncer entirely: anyone signed in, and anyone who pressed "I am a person" on the refusal page (a signed 30-day cookie set by a plain form, no JavaScript needed). The refusal page also links the contact and request-product pages and carries our phone and email; status 403, noindex. Refusals are logged (sampled, at most 20 lines a minute) so a whole browser family being refused by mistake shows up in Vercel logs under "bouncer". Owner rule (21 Aug 2026): no blanket old-browser or Windows 7 block, ever. Corpus test: scripts/test-bouncer.ts (47 cases). Fleets that send real client hints pass this layer; those are the Vercel Firewall's job (Challenge, rate limits)
- Ingest: /api/track, /api/search-log and /api/explore-log all reject requests matching the bot user-agent list or known crawler IP ranges (src/lib/bots.ts, ~60 patterns plus Googlebot/Bingbot IP prefixes); /api/track and /api/personal/rails refuse whatever the bouncer would refuse. The rails endpoint's compare_key lookup is one shared cached read (re-keyed by the catalogue watermark) instead of 4,000 rows per visitor.
